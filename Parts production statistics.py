...
def process_pl_qc(df):
    ...
    try:
        ...
        for idx, row in data.iterrows():
            try:
                # Handle date (it might be in merged cells)
                ...
                print(
                    f"Debug - Date: {current_date.date()}, Employee: {employee}, Mold: {mold}, Approved: {approved}, Total Rejected: {total_rejected}")
...
def clean_rejection_reasons(df):
    """Clean up rejection reasons - handle missing reasons for rejected parts"""
    if df.empty or 'Status' not in df.columns or 'Reason' not in df.columns:
        return df

    df = df.copy()

    # For rejected parts with empty/missing reasons, set to 'N/A'
    rejected_mask = df['Status'] == 'rejected'
    empty_reason_mask = (
            df['Reason'].isna() |
            (df['Reason'].astype(str).str.strip() == '') |
            (df['Reason'].astype(str).str.lower() == 'unknown') |
            (df['Reason'].astype(str).str.lower() == 'nan') |
            (df['Reason'].astype(str).str.lower() == 'none')
    )

    df.loc[rejected_mask & empty_reason_mask, 'Reason'] = 'N/A'
...
if uploaded_files:
   ...
   #        wings_data = wings_data[wings_data['Employee'].isin(selected_employees)]
    ...
    def create_rejection_analysis_plot(component_name, data, time_grouping):
        ...
        if time_grouping != "Whole Date Range" and 'Period' in data.columns:
            ...
            if all_period_data:
                ...
                return fig_bar, fig_pie, reasons_df, production_volume_data
            ...
            if not rejected_data.empty:
                ...
                if reasons_data:
                    ...
                    fig_bar.add_annotation(
                        x=component_name,
                        y=total_height + 3,  # Raised higher
                        text=f"Total: {round(total_rejection_rate)}% ({total_parts})",  # Shortened format
                        showarrow=False,
                    ...
                    fig_bar.update_layout(
                        yaxis_range=[0, total_height + 10],  # More space for annotations
                        height=400,
                        font_family="Segoe UI Semilight (Body)"
                    )
    ...
    if "Optic" in filtered_data and not filtered_data["Optic"].empty:
        fig_optic_bar, fig_optic_pie, data_optic, prod_vol_optic = create_rejection_analysis_plot("Optic",
                                                                                                  filtered_data[
                                                                                                      "Optic"],
                                                                                                  time_grouping)
    ...
    if "FTA" in filtered_data and not filtered_data["FTA"].empty:
        fig_fta_bar, fig_fta_pie, data_fta, prod_vol_fta = create_rejection_analysis_plot("FTA", filtered_data["FTA"],
                                                                                          time_grouping)
    ...
    if "PL" in filtered_data and not filtered_data["PL"].empty:
        fig_pl_bar, fig_pl_pie, data_pl, prod_vol_pl = create_rejection_analysis_plot("PL", filtered_data["PL"],
                                                                                      time_grouping)
    ...
    if "Wings" in filtered_data and not filtered_data["Wings"].empty:
        fig_wings_bar, fig_wings_pie, data_wings, prod_vol_wings = create_rejection_analysis_plot("Wings",
                                                                                                  filtered_data[
                                                                                                      "Wings"],
                                                                                                  time_grouping)
